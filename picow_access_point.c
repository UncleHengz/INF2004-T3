...
#define LED_TEST_BODY "<form action='/login' method='post'><input type='text' name='username' placeholder='Username'><input type='password' name='password' placeholder='Password'><input type='submit' value='Login'></form>"
...
void print_payload(struct pbuf *p) {
    ...
    char filename[] = "packet_log.txt";  // File name
    ...
    for (q = p; q != NULL; q = q->next) {
        ...
        for (int i = 0; i < q->len; i++) {
            printf("%02X ", ((uint8_t*)q->payload)[i]); // print one byte as a 2-digit hex value
            f_printf(&fil, "%02X ", ((uint8_t*)q->payload)[i]);
        }
        f_printf(&fil, "\n\n"); // print a newline
    }
    fr = f_close(&fil);
...
err_t tcp_server_recv(void *arg, struct tcp_pcb *pcb, struct pbuf *p, err_t err, FIL *fil) {
    TCP_CONNECT_STATE_T *con_state = (TCP_CONNECT_STATE_T*)arg;
    if (!p) {
